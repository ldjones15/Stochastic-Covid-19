function [Smin,Smax,Smean,Imin,Imax,Imean,Mmin,Mmax,Mmean,Rmin,Rmax,Rmean,ImaxDist,MDist,TotIDist,plot_time,kapl_plot,kaps_plot,LAM_plot,EndTime,gam]=GetProjectionsSlim(Stats,Tth,Tend,err,pop,tau,Itol,Strue,targidx,AddTime)
...
for i=1:N
	...
	KAPL=Stats(i,8);%KAPL=0; %SEIR model test
	KAPS=Stats(i,9); %KAPS=0; %SEIR model test
	LamMask=Stats(i,10);%LamMask=1; %SEIR model test
	Zeta=Stats(i,11);
	LockRel=Stats(i,12);
    Smooth=Stats(i,13);
